'''
Task: Программа расчета годового километража велосипедистов
Author: Алексей Гладкий
Date: 23.02.2025
'''
import cyclist_logic as cl, cyclist_list

def main():
    menu = """
    1 - добавить нового велосипедиста
    2 - добавить дистанцию
    3 - рассчитать километраж за год
    4 - вывести список велосипедистов
    5 - удалить велосипедиста
    -1 - выход\n
    Выбрать пункт меню: """



    operation = int(input(menu))  # Пользователь выбирает операцию.

    while operation != -1:  # Условие выхода из программы.
        if operation == 1:
            cl.add_new_cyclist()  # Добавление велосипедиста.
        elif operation == 2:
            cl.add_new_distance()  # Добавление дистанции.
        elif operation == 3:
            cl.calculation_avg_distance()  # Расчет годового километража.
        elif operation == 4:
            cyclist_list.show_cyclists()
        elif operation == 5:
            cl.delete_cyclist()
        operation = int(input(menu))


if __name__ == '__main__':
    print('main.py was started')
    main()
